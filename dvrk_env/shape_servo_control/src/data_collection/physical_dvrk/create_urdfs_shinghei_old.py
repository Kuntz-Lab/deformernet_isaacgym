...
# mesh_name = "multi_cylinders_1kPa"
# shape_name = "cylinder"
...
for i in range(1):
    ...
    height = temp
     ...
     # get coordinates of random attachment points
    max_num_balls = 3
    rand_num_balls = np.random.randint(low=1, high=max_num_balls+1)
    epsilon = min(height, width)/5
    balls_xy = get_random_balls(num_balls=rand_num_balls, xy_lower_bounds=[-(width/2),-(height/2-epsilon)], xy_upper_bounds=[width/2,height/2-epsilon], min_separation=attach_dist)
    ...
    <robot name="{object_name}">
        <link name="{object_name}">    
            <fem>
                ...
                <damping value="0.0" /> 
                <attachDistance value="{attach_dist+0.001}"/>
